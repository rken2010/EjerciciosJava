...
public class Partido {
	...
	public String mostrarResultado() {
		...
		for (int i = 0; i < eventosDelPartido.size(); i++) {
			if (eventosDelPartido.get(i).getEvento() == ListaEventos.GOL
					&& eventosDelPartido.get(i).getJugadorDelEvento().isLocal() == true) {
				golesLocales++;
			} else {
				golesVisitantes++;
			}
			...
			for (int i = 0; i <= equipoVisitante.size(); i++) {
				if (eventosDelPartido.get(i).getEvento() == ListaEventos.AMONESTACION) {
					if (eventosDelPartido.get(i).getJugadorDelEvento().getDNI() == jugadorAmonestado.getDNI()) {
						...
						Eventos amonestacion = new Eventos(jugadorAmonestado, ListaEventos.AMONESTACION,
								minutoAmonestacion);
